...
async function buyNow() {
    ...
    try {
        ...
        if (isECard) {
            // Create a single PNG for eCards
            const finalCanvas = document.createElement('canvas');
            finalCanvas.width = 1134; // Two canvases side by side
            finalCanvas.height = 794;
            const ctx = finalCanvas.getContext('2d');

            await createSecureCanvasCopy('front-canvas', ctx, 0, 0);
            await createSecureCanvasCopy('inner-right-canvas', ctx, 567, 0);

            // Convert to Blob and initiate download
            const imageBlob = await new Promise(resolve => finalCanvas.toBlob(resolve, 'image/png'));
            downloadBlob(imageBlob, `${currentCardData.cardType}-${Date.now()}.png`);
        } else {
            // Create a 2-page PDF for printable cards
            const pdf = new jsPDF({ unit: 'px', format: [1134, 1588] });

            const firstPageCanvas = document.createElement('canvas');
            firstPageCanvas.width = 1134;
            firstPageCanvas.height = 794;
            const firstPageCtx = firstPageCanvas.getContext('2d');

            await createSecureCanvasCopy('front-canvas', firstPageCtx, 0, 0);
            await createSecureCanvasCopy('back-canvas', firstPageCtx, 567, 0);
            pdf.addImage(firstPageCanvas.toDataURL('image/png'), 'PNG', 0, 0, 1134, 794);

            pdf.addPage();

            const secondPageCanvas = document.createElement('canvas');
            secondPageCanvas.width = 1134;
            secondPageCanvas.height = 794;
            const secondPageCtx = secondPageCanvas.getContext('2d');

            await createSecureCanvasCopy('inner-left-canvas', secondPageCtx, 0, 0);
            await createSecureCanvasCopy('inner-right-canvas', secondPageCtx, 567, 0);
            pdf.addImage(secondPageCanvas.toDataURL('image/png'), 'PNG', 0, 0, 1134, 794);
        ...
        alert(error.message.includes('Insufficient balance') ? 'Insufficient credits. Please add more credits to your account.' : 'Purchase failed. Please try again.');
    }
}
...
async function createSecureCanvasCopy(sourceId, targetCtx, x, y) {
    ...
    if (imgData) {
        await drawImage(tempCtx, imgData.src, imgData.x, imgData.y, imgData.width, imgData.height);
    }

    drawTexts(tempCtx, sourceId.replace('-canvas', ''));
    await drawStickers(tempCtx, sourceCanvas);

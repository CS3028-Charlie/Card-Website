...
function generateCards() {
    ...
    allCardsImages.forEach((cardImages, index) => {
        ...
        viewButton.style.fontSize = '16px';   // Adjust font size
        viewButton.style.padding = '5px 40px';  // Adjust padding
...
function isTextModerated(text) {
    return blacklist.some(badWord => text.toLowerCase().includes(badWord.toLowerCase()));
}

// Add Text Function
function addText(text, x, y) {
    if (isTextModerated(text)) {
        alert('Your text contains inappropriate content and cannot be added.');
        return; // Prevent adding the text
    }
...
document.getElementById('addTextBtn').addEventListener('click', () => {
    addText('Text Here', 50, 50); // Add text box with default text 'text'
});
...
canvas.addEventListener('keydown', (e) => {
    if (selectedText) {
        ...
        if (e.key === 'Backspace') {
            newText = newText.slice(0, cursorPosition - 1) + newText.slice(cursorPosition);
            ...
            newText = newText.slice(0, cursorPosition) + e.key + newText.slice(cursorPosition);
            cursorPosition++;
        ...
        if (isTextModerated(newText)) {
            alert('Your text contains inappropriate content and cannot be updated.');
            return; // Prevent updating the text
        }

        selectedText.text = newText; // Update the selected text
        redrawCanvas();
        updateTextBoxPosition();
